"""OpenCV UI helpers for Jetson demo."""
import cv2
...
def button_rects(frame_w, frame_h):
...
def hit_button(x, y, rect):
    x1, y1, x2, y2 = rect
    return x1 <= x <= x2 and y1 <= y <= y2


def draw_button(frame, rect, label, active, enabled=True):
    x1, y1, x2, y2 = rect
    if not enabled:
...
def draw_controls(frame, detecting):
    h, w = frame.shape[:2]
...
def draw_paused_banner(frame):
    banner = "PAUSED — click START or press S / Space"
    tw, th = cv2.getTextSize(banner, cv2.FONT_HERSHEY_SIMPLEX, 0.85, 2)[0]
    cv2.rectangle(
        frame,
...
def draw_verdict(frame, verdict, color):
    if not verdict:
        return
    tw, th = cv2.getTextSize(verdict, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 2)[0]
    cv2.rectangle(
        frame,
...
def draw_legend(frame, legend):
    for i, (text, color) in enumerate(legend):
        cv2.putText(frame, text, (12, 30 + i * 28), cv2.FONT_HERSHEY_SIMPLEX, 0.65, color, 2)


def draw_m1_poly(frame, poly, color):
    cv2.polylines(frame, [poly.astype("int32")], True, color, 2)


def draw_m2_hits(frame, hits):
    for hit in hits:
        color = M2_COLORS.get(hit.class_id, (255, 255, 255))
        cv2.polylines(frame, [hit.polygon.astype("int32")], True, color, 2)
