...
class RecyclingSession:
    def __init__(self, countdown_time=5.0, qr_display_time=30.0, demo_mode=False):
        ...
        # State: "idle" | "detecting" | "accepted" | "rejected" | "countdown" | "loading" | "qr_display"
        self.state = "idle" 
    ...
    def process_frame(self, detections, conf_threshold, component_inspection=None):
        ...
        if component_inspection is not None:
        ...
        if self.state == "idle":
            ...
            elif elapsed >= self.reject_display_time:
                self.transition("detecting" if self.demo_mode else "idle")
