# Model 1 RVM specialization — review draft and overnight execution contract

Date: 2026-09-06. Status: PLAN ONLY; execution has not been authorized.
...
Improve whole-object aluminum-can versus PET-bottle detection in the fixed RVM camera, particularly reflective steel, specular glare, transparent PET, bright backlighting, and dim exposure. Make reviewed real machine images the dominant training signal; expand training with controlled augmentation and suitable legacy replay. Preserve the current three-class HBB model and PET-only Model 2 routing.

This document is a proposed command for later execution, not permission to run now. Only branch creation, source inspection, and this plan are authorized in this turn. After the owner reviews and adjusts the plan, require an explicit command such as: `Execute the approved MODEL1_RVM_FINETUNE_PLAN.md on plan/model1-rvm-finetune with GPT-5.6 Luna High overnight.` Approval to execute includes implementing the scripts/tests and running the bounded candidate workflow described here. It does not approve production replacement, merge, remote publication, new downloads of third-party datasets, or killing unrelated processes.
...
- Task: `detect`; canonical classes `0=metal_can, 1=pet_bottle, 2=pp_cup`. Determine the actual backbone from the loaded checkpoint; do not infer YOLO architecture from directory names or switch architectures silently.
...
4. Model-assisted boxes may be generated as proposals in a separate directory. A reviewer must approve/correct classes and geometry before ingestion; teacher confidence alone is not ground truth. Review all can class-3 records too. Part-box union is not a reliable whole bottle extent.
...
For each machine TRAIN original, retain the original and create at most six deterministic variants, each generated directly from that original (never augment an augmented image):
...
For legacy TRAIN originals, create at most two direct variants matching training-machine luminance/color/noise and modest pose/framing statistics. Keep an untouched replay portion to resist forgetting. Simple color transfer cannot reproduce RVM geometry or transparent PET optics: do not claim domain equivalence. Do not paste rectangular cutouts or invent segmentation masks. Background compositing is deferred unless reviewed object masks and TRAIN-only empty backgrounds are available.
...
- Start from the verified imported M1 checkpoint, task=detect, classes exactly metal_can/pet_bottle/pp_cup. Preserve architecture and three-class head; no OBB model, no classifier stage, no new pretrained download.
...
Export the selected checkpoint into the candidate directory only: fixed batch 1 ONNX FP32 at 640 and 416, using the same tested export options as current M1. Validate inputs `[1,3,S,S]`, three class channels and seven output channels; compare output orientation explicitly rather than assuming it. Preserve class order and avoid double sigmoid.
...
The runner must refuse full training until an annotation-review manifest and frozen split pass validation. Audit/prepare/smoke modes must not accidentally fall through into full training. Use the existing `training/model1/.venv/Scripts/python.exe`; no environment installs or changes without first establishing they are needed and recording the exact change.

## 12. Luna execution handoff and morning deliverable

After explicit approval: read this entire document and owner amendments, verify branch, implement the minimal workflow and tests, perform audit/preflight, then execute only the phases whose inputs pass. Keep progress in local files. Do not delegate or change model automatically. If annotations/data are missing, complete the actionable audit and stop before training; never use pseudo-labels to bypass review. Respect overall deadline and bounded recovery limits.
