"""Generate Ultralytics reference outputs for validation contracts (dev only)."""
...
def main() -> int:
    ...
    ap.add_argument("--profile", default="main", choices=["main", "trial"])
    ap.add_argument("--models-root", default="MachineCode/models")
    args = ap.parse_args()

    models = ROOT / args.models_root
    det = models / "m1_detector_416.onnx"
    cls = models / "m1_classifier_224.onnx"
    m2 = models / "m2_obb_640.onnx"
    if not m2.is_file():
        m2 = models / "m2_obb_416.onnx"
    if not det.is_file() or not cls.is_file() or not m2.is_file():
        print(f"ERROR: missing ONNX under {models}", file=sys.stderr)
        return 1

    contract: dict = {"profile": args.profile, "fixtures": {}}
    ...
    out = CONTRACTS / f"{args.profile}.json"
